...
describe('/api/categories', () => {
  ...
  describe('GET', () => {
    it('should return categories list', async () => {
      const request = JestTestHelpers.createMockRequest('/api/categories');
      const response = await GET(request);
      const data = await response.json();

      if (response.status === 500) {
        console.log('500 Error Response (categories):', data);
    ...
    it('should handle search parameter', async () => {
      const request = JestTestHelpers.createMockRequest('/api/categories?search=Economy');
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should handle pagination', async () => {
      const request = JestTestHelpers.createMockRequest('/api/categories?page=1&limit=5');
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should include category statistics', async () => {
      const request = JestTestHelpers.createMockRequest('/api/categories?withStats=true');
      const response = await GET(request);
      const data = await response.json();

      if (response.status === 500) {
        console.log('500 Error Response (stats):', data);
        expect(data).toHaveProperty('success', false);
        expect(data).toHaveProperty('error');
        return;
      }
