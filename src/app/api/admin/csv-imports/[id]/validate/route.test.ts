...
describe('/api/admin/csv-imports/[id]/validate', () => {
  beforeEach(async () => {
...
Alaska,2023,Economy,GDP,50000`;
      ...
      formData.append('metadata', JSON.stringify({ description: 'Test validation' }));
      ...
      const request = JestTestHelpers.createMockRequest(`/api/admin/csv-imports/${importId}/validate`, {
        method: 'POST'
      });
...
Alabama,invalid_year,Economy,GDP,50000`;

      const formData = new FormData();
      const file = new File(invalidCSV, 'test.csv', { type: 'text/csv' });
      formData.append('file', file);
      formData.append('templateId', '1');
      formData.append('metadata', JSON.stringify({ description: 'Test invalid validation' }));
      formData.append('userId', '1');
      ...
      if (importId) {
        // Try to validate the failed import
        const request = JestTestHelpers.createMockRequest(`/api/admin/csv-imports/${importId}/validate`, {
          method: 'POST'
        });
        const response = await POST(request, { params: Promise.resolve({ id: importId.toString() }) });
    ...
    it('should handle invalid import ID', async () => {
      const request = JestTestHelpers.createMockRequest('/api/admin/csv-imports/999/validate', {
        method: 'POST'
      });
    ...
    it('should handle non-numeric import ID', async () => {
      const request = JestTestHelpers.createMockRequest('/api/admin/csv-imports/invalid/validate', {
        method: 'POST'
      });
