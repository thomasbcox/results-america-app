...
export async function POST(
  ...
  try {
    ...
    const result = await UnifiedCSVImportService.rollbackPromotion(importId, userId);

    if (result.success) {
      return createSuccessResponse({
        rolledBackRows: result.rolledBackRows
      }, result.message);
    } else {
    ...
    return createErrorResponse(new ServiceError('Rollback failed', 'ROLLBACK_ERROR', 500));
  }
} 