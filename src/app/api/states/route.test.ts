...
describe('/api/states', () => {
  ...
  describe('GET', () => {
    it('should return states list', async () => {
      const request = JestTestHelpers.createMockRequest('/api/states');
      const response = await GET(request);
      const data = await response.json();

      if (response.status === 500) {
        console.log('500 Error Response (states):', data);
    ...
    it('should handle search parameter', async () => {
      const request = JestTestHelpers.createMockRequest('/api/states?search=California');
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should handle pagination', async () => {
      const request = JestTestHelpers.createMockRequest('/api/states?page=1&limit=5');
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should include state abbreviations', async () => {
      const request = JestTestHelpers.createMockRequest('/api/states');
      const response = await GET(request);
      const data = await response.json();

      if (response.status === 500) {
        console.log('500 Error Response (abbreviations):', data);
        expect(data).toHaveProperty('success', false);
        expect(data).toHaveProperty('error');
        return;
      }
